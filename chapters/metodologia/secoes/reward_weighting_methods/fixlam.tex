...
The static reward weighting function, denoted as \gls{udc}, serves as a fundamental step toward our dynamic formulation, DyLam. This method extends the classical Reinforcement Learning framework by shifting the responsibility of combining reward components from the environment to the agent. We first revisit the $Q$-decomposition of~\cite{russell2003q} and introduce a \emph{realistic control} variant of the $Q$-learning update, in contrast to the commonly adopted \emph{illusion of control} formulation. We then present the core idea of decoupling reward weights from reward components and subsequently extend the formulation to Policy Gradient methods.
...
We recall the global action-value function~\cite{russell2003q}:
\begin{equation}
...
Following~\citeonline{russell2003q}, the component-wise $Q$-updates satisfy:
\begin{gather}
    \begin{split}
        Q_i^\pi(s_t, a_t) = (1-\alpha_i) Q_i^\pi(s_t, a_t)
        + \alpha_i \big[
            r_i(s_t, a_t, s_{t+1})
...
As discussed in Section~\ref{sec:decompq}, this induces an \emph{illusion of control}: each component behaves as an independent sub-agent optimizing only its own reward. While consistent with the multi-agent interpretation presented by~\citeonline{russell2003q}, it is incompatible with the single-agent perspective of this work.

\subsubsection{Unified Decision Control for Q-Learning}
...
\begin{definition}[Unified Decision Policy]
    The unified greedy policy is defined as
    \begin{equation}
...
and the global value function is reconstructed through Eq. \ref{eq:QG_def}. Unlike Eq.~\eqref{eq:zimdars_original}, where each $Q_i$ selects its own greedy action independently, Eq.~\eqref{eq:realistic_component_update} enforces that all components are evaluated under the \textbf{same} next action $\pi_G(s_{t+1})$, ensuring that action selection is governed by the global objective rather than by local reward components.

\subsubsection{Unified Decomposition and Global Consistency}

We now show that the unified update recovers the standard Q-learning update for the global value function.

\begin{theorem}[Consistency of Unified Decision Q-Learning]
...
\begin{corollary}[Convergence of Unified Decision Q-Learning]
...
\begin{proof}
Since Theorem~\ref{theo:realistic_q_learning} shows that $Q_G$ satisfies the Bellman optimality operator for standard Q-learning, convergence follows directly from~\citeonline{watkins1989learning}.
\end{proof}
...
Assuming a constant $\lambda$ vector and substituting each $\lambda_i Q^\pi_i (s,a)$ by $Q'^\pi_i (s,a)$ in Equation \ref{eq:ext_pg_theo} we obtain:
