...
Many value-based and policy gradient methods are built upon the TD-learning framework. They extend the TD idea to estimate action-value functions, which are then used to derive policies~\cite{sutton2018reinforcement}. While the specific methods explored in this work are rooted in the TD framework, we note that our final approach is Monte Carlo-based. In Chapter~\ref{chap:method}, we explain how our method requires a large number of episodes ($E \gg 1$) to ensure stable convergence and performance.
